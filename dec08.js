...
const parseInput = (input) => {
  ...
  for (const line of lines) {
    const [rawObservations, rawAnswer] = line.split(" | ");
    let observations = rawObservations
      .trim()
      .split(" ")
      .map((x) => x.split("").sort().join(""));
    observations = _.uniq(observations);
    let output = rawAnswer
      .trim()
      .split(" ")
      .map((x) => x.split("").sort().join(""));
    output = output;
    let all = [...observations, ...output];
    result.push({ observations, output });
  }
...
const partOne = (input) => {
  ...
  for (const line of parsed) {
    const ones = line.signal.filter((x) => x.length === 2).length;
    const fours = line.signal.filter((x) => x.length === 4).length;
    const sevens = line.signal.filter((x) => x.length === 3).length;
    const eights = line.signal.filter((x) => x.length === 7).length;
...
// returns true if all of segmentB is in segmentA
const includes = (segmentA, segmentB) => {
  const elementsA = segmentA.split("");
  const elementsB = segmentB.split("");
  for (const element of elementsB) {
    if (!elementsA.includes(element)) {
      return false;
    }
  }
...
const partTwo = (input) => {
  ...
  let result = 0;
  for (const line of parsed) {
    // 1, 4, 7, and 8 are straightforward.
    const _1 = line.observations.filter((x) => x.length === 2)[0];
    const _4 = line.observations.filter((x) => x.length === 4)[0];
    const _7 = line.observations.filter((x) => x.length === 3)[0];
    const _8 = line.observations.filter((x) => x.length === 7)[0];

    // 5, 2, and 3 all have 5 entries, I'll call this the "alpha group".
    let alpha = line.observations.filter((x) => x.length === 5);

    // 0, 9, and 6 all have 6 entries, I'll call these the "beta group".
    let beta = line.observations.filter((x) => x.length === 6);

    // 3 is the only alpha that contains 1's segments.
    const _3 = alpha.filter((x) => includes(x, _1))[0];
    alpha = alpha.filter((x) => x !== _3);

    // 9 is the only beta that contains 4's segments.
    const _9 = beta.filter((x) => includes(x, _4))[0];
    // 6 is the only beta that contains 7's segments.
    const _6 = beta.filter((x) => !includes(x, _7))[0];
    // ...which leaves zero as the lone remaining beta!
    const _0 = beta.filter((x) => !_.includes([_6, _9], x))[0];

    // Now, the only alpha that 6 includes is 5
    const _5 = alpha.filter((x) => includes(_6, x))[0];
    // ...which leaves 2 as the lone remaining alpha!
    const _2 = alpha.filter((x) => !_.includes([_5], x))[0];

    // Now that we know the numbers, we'll need to get the
    const lookupList = [_0, _1, _2, _3, _4, _5, _6, _7, _8, _9];
    const getNumericValueString = (code) => {};
    const outputValue = line.output
      .map((code) => {
        // Get the numeric value (e.g. 'ab' -> '1'
        return `${_.indexOf(lookupList, code)}`;
      })
      .join(""); // ...then join the digits together into e.g. '1234';

    // ...and add that output to the total
    result += parseInt(outputValue);
  }
  return result;
};
...
console.log(`partTwo known: ${partTwo(SINGLE)}`);
